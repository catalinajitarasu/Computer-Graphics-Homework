...
void Display4() {
    double xmax, ymax, xmin, ymin , x,y, yFin, yNext, xNext;
    double a = 0.2;
    double pi = 4 * atan(1.0);
   ...
   for (double t = -pi / 2 + ratia; t < -pi / 6; t += ratia) {
    ...
    for (double t = -pi / 2 + ratia1; t < -pi / 6; t += ratia1) {

        double x = a / (4 * cos(t) * cos(t) - 3);
        double y = (a * tan(t)) / (4 * cos(t) * cos(t) - 3);

        double xNext = a / (4 * cos(t + ratia1) * cos(t + ratia1) - 3);
        double yNext = (a * tan(t + ratia1)) / (4 * cos(t + ratia1) * cos(t + ratia) - 3);

        if (x >= xStart && y <= yStart && y > 0.3) {
            if (counts % 3 == 2) {
                glVertex2f(x, y);
...
void Display6() {
    ...
    for (t = -pi / 4 + ratia; t < pi / 4; t += ratia) {
        double x1, y1;
        x1 = a * sqrt(2 * cos(2 * t)) * cos(t) / (xmax*2) ;
        y1 = a * sqrt(2 * cos(2 * t)) * sin(t) / (ymax*2);
    ...
    for (t = -pi / 4 + ratia; t < pi / 4; t += ratia) {
        double x2, y2;
        x2 = -a * sqrt(2 * cos(2 * t)) * cos(t) / (xmax*2);
        y2 = -a * sqrt(2 * cos(2 * t)) * sin(t) / (ymax*2);
