    ...
    void display(double xmin, double ymin, double xmax, double ymax)
    {
        ...
        for (double x = xmin; x <= xmax; x += RX_JF)
            for (double y = ymin; y <= ymax; y += RY_JF)
            {
                ...
                {
                    glColor3f( 1.0 / m.nriter, 1.0* r / m.nriter, 1.0*(1 + r) / m.nriter);
                    glVertex3d(x, y, 0);
...
public:
    void image1(double lungime, int nivel, CPunct& p, CVector& v)
    {
        ...
        else
        {
            for (int i = 0; i <= 3; i++) {
                ...
                v.rotatie(-i * 90);
                v.deseneazaPatrat(p1, lungime / 3);
    ...
    void afisare(double lungime, int nivel)
    {
        ...
        CVector v(0.0, 1.0);
        CPunct p(0.0, 0.0);
        v.deseneazaPatrat(p, 0.4);
        image1(0.4, nivel, p, v);
...
class CImage3 {
public:
    void image3(double lungime, int nivel, CPunct& p, CVector& v, double d)
    {
        CPunct p1;
        if (nivel == 0)
        {
